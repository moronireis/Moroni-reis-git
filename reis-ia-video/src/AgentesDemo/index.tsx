...
import transcriptData from "../../public/captions/agentes-demo-transcript.json";
...
// Scene boundaries derived from transcript segments (seconds)
// Each scene groups 1+ segments and maps to a visual
const SCENES = [
  { id: "hook",     start: 0,     end: 4.5,   segs: [0] },
  { id: "grid",     start: 4.5,   end: 10.6,  segs: [1] },
  { id: "site",     start: 10.6,  end: 12.8,  segs: [2] },
  { id: "copy",     start: 12.8,  end: 14.6,  segs: [3] },
  { id: "ads",      start: 14.6,  end: 17.3,  segs: [4, 5] },
  { id: "strategy", start: 17.3,  end: 20.8,  segs: [6] },
  { id: "dev",      start: 20.8,  end: 23.2,  segs: [7] },
  { id: "montage",  start: 23.2,  end: 26.2,  segs: [8] },
  { id: "payoff",   start: 26.2,  end: 29.0,  segs: [9, 10] },
  { id: "price",    start: 29.0,  end: 38.5,  segs: [10, 11] },
  { id: "install",  start: 38.5,  end: 43.0,  segs: [12, 13] },
  { id: "cta",      start: 43.0,  end: 50.0,  segs: [14] },
] as const;
  ...
  return (
    <AbsoluteFill style={{ justifyContent: "center", alignItems: "center", flexDirection: "column", gap: 28, padding: "60px 50px" }}>
      ...
      <div style={{ display: "flex", alignItems: "baseline", gap: 10, opacity: ease(frame, 0, 1, 35, 50) }}>
        ...
        <span style={{ fontFamily: SANS, fontSize: 20, fontWeight: 400, color: WHITE_DIM }}>agentes prontos</span>
      </div>
    </AbsoluteFill>
  );
}
...
function SiteScene() {
  return (
    <AbsoluteFill style={{ justifyContent: "center", alignItems: "center", padding: "0 50px" }}>
      <Terminal command='@dev-agent "Landing page para oferta de R$497"' output={["Design system dark mode...", "Hero + seções + checkout...", "✓ Site completo — 12 min"]} typingSpeed={1} badge="DEPLOYED" />
    </AbsoluteFill>
  );
}
...
function CopyScene() {
  return (
    <AbsoluteFill style={{ justifyContent: "center", alignItems: "center", padding: "0 50px" }}>
      <Terminal command='@copy-squad "Página Hormozi 4 ângulos"' output={["Copywriter → Humanizer → Reviewer...", '✓ "Pare de explicar. Comece a faturar."']} typingSpeed={1} badge="APPROVED" badgeColor={BLUE} />
    </AbsoluteFill>
  );
}
...
function AdsScene() {
  return (
    <AbsoluteFill style={{ justifyContent: "center", alignItems: "center", padding: "0 50px" }}>
      <Terminal command='@traffic-manager "Campanha tráfego frio R$50/dia"' output={["3 ad sets segmentados...", "Criativos uploaded...", "✓ Campanha criada"]} typingSpeed={1} badge="PAUSED" badgeColor="#eab308" />
    </AbsoluteFill>
  );
}
...
function StrategyScene() {
  return (
    <AbsoluteFill style={{ justifyContent: "center", alignItems: "center", padding: "0 50px" }}>
      <Terminal command='@market-research "Top 3 concorrentes"' output={["Posicionamento mapeado...", "Preços e canais analisados...", "✓ 5 gaps identificados"]} typingSpeed={1} badge="REPORT READY" />
    </AbsoluteFill>
  );
}
...
function DevScene() {
  return (
    <AbsoluteFill style={{ justifyContent: "center", alignItems: "center", padding: "0 50px" }}>
      <Terminal command='@integration-engineer "Webhook leads"' output={["Endpoint + RLS + notificação...", "✓ Pipeline ativo"]} typingSpeed={1} badge="DEPLOYED" />
    </AbsoluteFill>
  );
}
  ...
  return (
    <AbsoluteFill style={{ justifyContent: "center", alignItems: "center", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ position: "relative", opacity: ease(frame, 0, 1, 5, 18) }}>
        <span style={{ fontFamily: SANS, fontSize: 48, fontWeight: 300, color: "rgba(255,80,80,0.6)" }}>R$21.000/mês</span>
      ...
      <div style={{ opacity: newOp, transform: `scale(${interpolate(newScale, [0, 1], [0.7, 1])})`, display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
        <span style={{ fontFamily: SANS, fontSize: 88, fontWeight: 800, color: WHITE, letterSpacing: "-0.04em" }}>R$47</span>
      ...
      <div style={{ display: "flex", gap: 16, marginTop: 16, opacity: badgeOp, flexWrap: "wrap", justifyContent: "center", padding: "0 40px" }}>
        {["41 agentes", "7 squads", "4 videoaulas", "Garantia 7 dias"].map((b) => (
          <div key={b} style={{
...
export const AgentesDemo: React.FC = () => {
  const fps = 30;
  const totalDuration = 50; // seconds
  ...
  return (
    <AbsoluteFill style={{ background: "#000" }}>
      ...
      <Audio src={staticFile("audio/agentes-demo-vo.mp3")} volume={1} />

      {/* Scenes synced to transcript timestamps */}
      {SCENES.map((scene) => {
