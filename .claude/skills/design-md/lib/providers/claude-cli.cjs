...
function invoke(promptText, options = {}) {
  ...
  // Strip ANTHROPIC_API_KEY so claude -p uses the logged-in plan (Max/Pro)
  // instead of the API credit balance which may be empty.
  ...
  const result = spawnSync(
    ...
    ],
    {
      ...
      timeout: 0, // no timeout — let the plan rate limit handle it
      encoding: "utf8",
    }
  );
